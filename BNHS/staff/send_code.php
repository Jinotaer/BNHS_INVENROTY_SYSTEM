...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = $mysqli->prepare("SELECT * FROM bnhs_staff WHERE staff_email = ?");
  ...
  if ($user) {
    ...
    $update = $mysqli->prepare("UPDATE bnhs_staff SET resetcode = ?, created_at = NOW() WHERE staff_email = ?");
    $update->bind_param('ss', $reset_code, $staff_email);
    $update->execute();
