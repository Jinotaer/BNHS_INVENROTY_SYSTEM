...
if (isset($_POST['ChangeProfile'])) {
  ...
  $Qry = "UPDATE bnhs_admin SET admin_name =?, admin_email =? WHERE admin_id =?";
...
if (isset($_POST['changePassword'])) {
  ...
  if (!$error) {
    $admin_id = $_SESSION['admin_id'];
    // Correct the table name to 'bnhs_admin' for password verification
    $sql = "SELECT * FROM bnhs_admin WHERE admin_id = '$admin_id'";
    ...
    if (mysqli_num_rows($res) > 0) {
      ...
      if ($old_password != $row['admin_password']) {
        ...
        // Update the password in the 'bnhs_admin' table
        $query = "UPDATE bnhs_admin SET admin_password =? WHERE admin_id =?";
